...
const createMyDoctorSchedule = async (user: IUserRequest, payload: ICreateDoctorSchedulePayload) => {
...
const updateMyDoctorSchedule = async (user: IUserRequest, payload: IUpdateDoctorSchedulePayload) => {
    const doctorData = await prisma.doctor.findUniqueOrThrow({
        where: {
            email: user.email
        }
...
const deleteMyDoctorSchedule = async (id: string, user: IUserRequest) => {
    const doctorData = await prisma.doctor.findUniqueOrThrow({
        where: {
            email: user.email
        }
